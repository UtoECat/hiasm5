...
//--------------------------------------------------------------------------------------------------
ElementWinElement::ElementWinElement(PackElement *pe, SDK *sdk, gdouble x, gdouble y) : Element(pe, sdk, x, y) { cIndex = CI_WinElement; }
...
ElementDynPoints::ElementDynPoints(PackElement *pe, SDK *sdk, gdouble x, gdouble y) : Element(pe, sdk, x, y) {
...
void ElementDynPoints::init() {
	...
	for (int i = 0; i < 4; i++)
		if (dynInfoList[i].prop) on_change_property(dynInfoList[i].prop);
}
		...
		if (dynInfoList[i].prop == prop) {
			...
			if (val < 0 || val > MAX_POINTS)
				...
				if (count < val) {	// add
					...
					if (count != val) prop->setValue(&count);
				}
				rePosPoints();
				r.join(drawRect());
...
ElementNamedDynPoints::ElementNamedDynPoints(PackElement *pe, SDK *sdk, gdouble x, gdouble y) : Element(pe, sdk, x, y) {
	int l = tpl->conf->sub.length();
	char *sub = new char[l + 1];
	tpl->conf->sub.copy(sub, l);
	sub[l] = '\0';
...
ElementMulti::ElementMulti(PackElement *pe, SDK *sdk, gdouble x, gdouble y) : Element(pe, sdk, x, y) {
	this->sdk = new SDK();
...
ElementMulti::~ElementMulti() {
	if (isELink()) {
		if (isMainELink()) {
			...
			if (elinkList->size() > 2) msdk->sdkLib.push_back(e);
		}
	} else
		delete sdk;
}
...
void ElementMulti::on_change_property(ElementProperty *prop) {
	...
	if (prop == sysProps[2]) {
		if (stockRender) icon = stockRender(prop->readStr());
		invalidate();
	}
}
...
int ElementEditMulti::isDynPointProp(ElementProperty *prop) {
	for (int i = 0; i < 4; i++)
		if (prop == dynInfoList[i].prop) return i;
	return -1;
}
...
void ElementEditMulti::changePoints(Element *multi, int i) {
	...
	if (c < val) {	// add
		for (int j = c; j < val; j++) multi->addPoint(dynInfoList[i].name + int_to_str(j + 1), "", mt_types[i]);
...
void ElementEditMultiEx::changePoints(Element *multi, int i) {
	...
	for (StringList::iterator line = list.begin(); line != list.end(); line++)
		if (!multi->points.getByName(*line)) {
			parseHintName(*line, n, h);
			if (n.length()) multi->addPoint(n, h, mt_types[i]);
		}
	multi->rePosPoints();
}
...
static const struct {
	const gchar *name;
	const gchar *info;
} sprops[] = {{"Width", "Width of element in editor"},
							{"Height", "Height of element in editor"},
							{"VOffset", "Points offset at top edge"},
							{"HOffset", "Points offset at left edge"}};

ElementEditMultiBase::ElementEditMultiBase(PackElement *pe, SDK *sdk, gdouble x, gdouble y) : Element(pe, sdk, x, y), ElementGrip() {
	for (int i = 0; i < 4; i++) sysProps.push_back(new ElementProperty(this, sprops[i].name, sprops[i].info, data_int));
...
void ElementEditMultiBase::on_change_property(ElementProperty *prop) {
	...
	if (i != -1) {
		if (parent->parent->isELink())
			for (ElementsList::iterator e = parent->parent->elinkList->begin(); e != parent->parent->elinkList->end(); e++) changePoints(*e, i);
		else
...
void ElementEditMultiBase::drawBody(DrawContext dc, double zoom) {
	...
	if (isSelect()) drawGrips(dc);

	dc->set_line_width(1);
}

bool ElementEditMultiBase::checkCollise(gdouble x, gdouble y) {
	if (isSelect() && getGripAtPos(x - this->x, y - this->y)) return true;
	if (Element::checkCollise(x, y)) return getStateAtPos(x, y) != -1;
	return false;
}

bool ElementEditMultiBase::checkColliseRect(gdouble x1, gdouble y1, gdouble x2, gdouble y2) {
	if (Element::checkColliseRect(x1, y1, x2, y2)) return !((x1 > x + 5) && (x2 < x + width - 5) && (y1 > y + 5) && (y2 < y + height - 5));
	return false;
}
...
void ElementEditMultiBase::rePosPoints() {
	Element::rePosPoints();
	for (ElementPoints::iterator p = points.begin(); p != points.end(); p++) switch ((*p)->type) {
			case pt_event:
				(*p)->pos->x -= width;
				(*p)->pos->y += *voff;
				break;
...
int ElementEditMultiBase::mouseCursor(gdouble x, gdouble y) {
	if (isActiveGrip()) return isActiveGrip()->cursor;

	GripInfo *g = getGripAtPos(x - this->x, y - this->y);
	if (isSelect() && g) return g->cursor;
...
ElementDynGeometry::ElementDynGeometry(PackElement *pe, SDK *sdk, gdouble x, gdouble y) : Element(pe, sdk, x, y), ElementGrip() {
	...
	for (int i = 0; i < 8; i++) {
		...
		if (i >= 2 && i <= 4) g->addValue(&width, 1, 0);
		if (i >= 4 && i <= 6) g->addValue(&height, 0, 1);
	}
	rePosGrids();
...
bool ElementDynGeometry::on_grip_can_change(GripInfo *grip, int &dx, int &dy) { return true; }

bool ElementDynGeometry::mouseDown(gdouble x, gdouble y, int button) {
	r = drawRect();
	dx = this->x;
	dy = this->y;
	if (isSelect() && gripDown(x - dx, y - dy)) return true;
	return false;
}

void ElementDynGeometry::mouseMove(gdouble x, gdouble y) { gripMove(x - dx, y - dy); }

void ElementDynGeometry::mouseUp(gdouble x, gdouble y, int button) { gripUp(x - dx, y - dy); }

int ElementDynGeometry::mouseCursor(gdouble x, gdouble y) {
	if (isActiveGrip()) return isActiveGrip()->cursor;
	if (isSelect())
		if (GripInfo *g = getGripAtPos(x - this->x, y - this->y)) return g->cursor;
	return 0;
}
...
void ElementInfoTip::on_change_property(ElementProperty *prop) {
	...
	if (prop->value == text || prop->value == formatText) {
		...
		if (textLayout) textLayout->set_alignment((Pango::Alignment)(*halign + Pango::ALIGN_LEFT));
	}
	invalidate();
}

bool ElementInfoTip::checkCollise(gdouble x, gdouble y) {
	if (ElementDynGeometry::checkCollise(x, y)) return true;
...
void ElementDebug::on_change_property(ElementProperty *prop) { Element::on_change_property(prop); }
...
void ElementDebug::draw(DrawContext dc, double zoom) {
	...
	if (isSelect()) drawPoints(dc);
}

void ElementDebug::remove() { removeFromLine(true, true); }

//--------------------------------------------------------------------------------------------------

ElementCore::ElementCore(PackElement *pe, SDK *sdk, gdouble x, gdouble y) : Element(pe, sdk, x, y) { flag |= ELEMENT_FLG_IS_CORE; }
...
ElementCoreDoData::ElementCoreDoData(PackElement *pe, SDK *sdk, gdouble x, gdouble y) : Element(pe, sdk, x, y), ElementCore(pe, sdk, x, y) {
	// none
}
...
void ElementCoreMulti::do_work(ElementPoint *point, TData *data) {
	...
	if (ElementPoint *p = child->points[i]->point) dynamic_cast<ElementCore *>(p->parent)->do_work(p, data);
}

void ElementCoreMulti::read_var(ElementPoint *point, TData *data) {
	int i = points.indexOf(point);
	if (ElementPoint *p = child->points[i]->point) dynamic_cast<ElementCore *>(p->parent)->read_var(p, data);
}
...
void ElementCoreEditMulti::do_work(ElementPoint *point, TData *data) {
	int i = points.indexOf(point);
	if (ElementPoint *p = parent->parent->points[i]->point) dynamic_cast<ElementCore *>(p->parent)->do_work(p, data);
}

void ElementCoreEditMulti::read_var(ElementPoint *point, TData *data) {
	int i = points.indexOf(point);
	if (ElementPoint *p = parent->parent->points[i]->point) dynamic_cast<ElementCore *>(p->parent)->read_var(p, data);
}
...
void ElementCoreStrCat::read_var(ElementPoint *point, TData *data) { *data = value; }
...
void ElementCoreFormatStr::read_var(ElementPoint *point, TData *data) { *data = _res; }

//--------------------------------------------------------------------------------------------------

ElementCoreCopy::ElementCoreCopy(PackElement *pe, SDK *sdk, gdouble x, gdouble y) : Element(pe, sdk, x, y), ElementCore(pe, sdk, x, y) {}
...
void ElementCoreCopy::read_var(ElementPoint *point, TData *data) { *data = _res; }
...
void ElementCoreReplace::do_work(ElementPoint *point, TData *data) {
	...
	ustring src = readString(data, points[4], props[0]), dst = readString(data, points[5], props[1]);
	int pos = 0;
...
void ElementCoreReplace::read_var(ElementPoint *point, TData *data) { *data = _res; }
...
void ElementCoreBlockFind::do_work(ElementPoint *point, TData *data) {
	if (point->name == "doSearch") {
		...
		while (i >= 0) {
			...
			if (j == -1) break;
			if (!props[0]->readInt())
				points[1]->on_event(ustring(text, i, j - i + _eb.size()));
			else
				points[1]->on_event(ustring(text, i + _sb.size(), j - i - _sb.size()));
			if (_stop) break;
			if (props[1]->readInt())
				j += _eb.size();
...
//--------------------------------------------------------------------------------------------------
ElementCoreFor::ElementCoreFor(PackElement *pe, SDK *sdk, gdouble x, gdouble y) : Element(pe, sdk, x, y), ElementCore(pe, sdk, x, y) {}

void ElementCoreFor::do_work(ElementPoint *point, TData *data) {
	if (point == points[0]) {
		if (props[4]->readInt()) data = NULL;
		i = readInteger(data, points[5], props[0]);
		e = readInteger(data, points[6], props[1]);
		int s = props[2]->readInt();
		if (!props[3]->readInt()) e += s;
		for (; i < e; i += s) points[2]->on_event(i);
		points[3]->on_event();
	} else
		i = e;
}

void ElementCoreFor::read_var(ElementPoint *point, TData *data) { *data = i; }

//--------------------------------------------------------------------------------------------------

ElementCoreIfElse::ElementCoreIfElse(PackElement *pe, SDK *sdk, gdouble x, gdouble y) : Element(pe, sdk, x, y), ElementCore(pe, sdk, x, y) {
	// none
}
...
ElementCoreRandom::ElementCoreRandom(PackElement *pe, SDK *sdk, gdouble x, gdouble y) : Element(pe, sdk, x, y), ElementCore(pe, sdk, x, y) {
	// none
}
...
void ElementCoreRandom::read_var(ElementPoint *point, TData *data) { *data = value; }

//--------------------------------------------------------------------------------------------------

ElementCoreMath::ElementCoreMath(PackElement *pe, SDK *sdk, gdouble x, gdouble y) : Element(pe, sdk, x, y), ElementCore(pe, sdk, x, y) {
	// none
}
...
void ElementCoreMath::read_var(ElementPoint *point, TData *data) { *data = value; }

//--------------------------------------------------------------------------------------------------

ElementCoreMemory::ElementCoreMemory(PackElement *pe, SDK *sdk, gdouble x, gdouble y) : Element(pe, sdk, x, y), ElementCore(pe, sdk, x, y) {
	// none
}

void ElementCoreMemory::prepareForRun(Element **parent) { value = *props[0]->readData(); }
...
void ElementCoreMemory::read_var(ElementPoint *point, TData *data) { *data = value; }

//--------------------------------------------------------------------------------------------------

ElementCoreTimer::ElementCoreTimer(PackElement *pe, SDK *sdk, gdouble x, gdouble y) : Element(pe, sdk, x, y), ElementCore(pe, sdk, x, y) {
	// none
}

void ElementCoreTimer::prepareForRun(Element **parent) {
	if (props[1]->readInt() == 0) start();
}

void ElementCoreTimer::stopRun() { stop(); }

gboolean _timer_proc(void *data) { return ((ElementCoreTimer *)data)->on_timer(); }
...
void ElementCoreChanelToIndex::read_var(ElementPoint *point, TData *data) { (*data) = remdt; }
...
void ElementCoreConsole::do_work(ElementPoint *point, TData *data) {
	for (ElementPoints::iterator p = points.begin(); p != points.end(); p++)
		if ((*p)->type == pt_data) std::cout << readString(data, *p, NULL).c_str();
	std::cout << std::endl;
	points[1]->on_event();
}
...
void ElementCoreClipboard::read_var(ElementPoint *point, TData *data) { *data = Clipboard::get()->wait_for_text(); }
...
void ElementCoreFileSearch::enumDir(const ustring &path, bool fn, int i) {
	...
	for (Dir::iterator d = dir.begin(); d != dir.end() && !stop; d++) {
		...
		if (i != 2 || subDir) isDir = file_test(fullName, FILE_TEST_IS_DIR);
		...
		if (outFile) points[2]->on_event(fn ? fullName : *d);

		if (subDir && isDir) enumDir(fullName, fn, i);
	}
}
...
void ElementCoreFileSearch::read_var(ElementPoint *point, TData *data) { *data = count; }
...
ElementHubEx::ElementHubEx(PackElement *pe, SDK *sdk, gdouble x, gdouble y) : Element(pe, sdk, x, y), ElementCore(pe, sdk, x, y) {
	minWidth = 9;
	minHeight = minWidth + 2;

	angle = (int *)props[0]->value;

	for (int i = 1; i < 4; i++) addPoint(ustring("doWork") + int_to_str(i), "Provide thread", pt_work);
...
void ElementHubEx::do_work(ElementPoint *point, TData *data) { points[3]->on_event(data); }
...
static const int hx_dirs[4][4] = {{1, 0, 0, 2}, {1, 0, 2, 0}, {0, 0, 1, 2}, {1, 0, 0, 2}};
...
void ElementHubEx::connectToPoint(ElementPoint *point) {
	...
	for (int i = 0; i < 3; i++)
		if (!points[i]->point) {
			...
			while (pp) {
				m++;
				if (pp->next) l += max(3, abs(pp->x - pp->next->x) + abs(pp->y - pp->next->y));
				pp = pp->next;
			}
			res[i] = (m << 16) + l;
			point->clear();
...
void ElementHubEx::remove() {
	...
	if (p) {
		PointPos *pp = p->pos;
		while (pp && (pp->x != _x || pp->y != _y)) pp = pp->next;
		if (pp == NULL) return;
...
ElementGetDataEx::ElementGetDataEx(PackElement *pe, SDK *sdk, gdouble x, gdouble y) : Element(pe, sdk, x, y), ElementHubEx(pe, sdk, x, y) {
	minHeight = 9;
...
void ElementGetDataEx::read_var(ElementPoint *point, TData *data) { points[3]->get_data(data); }
...
void ElementCoreEventFromData::do_work(ElementPoint *point, TData *data) { _dt = readData(data, points[3], NULL); }
...
void ElementCoreTransmitter::do_work(ElementPoint *point, TData *data) {
	for (int i = 0; i < props[0]->readInt(); i++) ret << readData(data, points[2 + i], NULL);
}
...
void ElementCoreStrList::prepareForRun(Element **parent) { list = props[1]->readStr(); }
...
void ElementCoreDS_SQLite::stopRun() { close(); }
...
void ElementCoreDS_SQLite::exec(const ustring &sql) { sqlite3_exec(db, sql.c_str(), NULL, NULL, NULL); }

sqlite3_stmt *ElementCoreDS_SQLite::query(const ustring &sql) {
	sqlite3_stmt *result;
	sqlite3_prepare(db, sql.c_str(), -1, &result, NULL);
	if (!result) std::cout << "Sql query " << sql.c_str() << " return empty result!" << std::endl;
	return result;
}
...
void ElementCoreDSC_Exec::read_var(ElementPoint *point, TData *data) { *data = (int)last_row; }
...
void ElementCoreDSC_QueryScalar::read_var(ElementPoint *point, TData *data) { *data = result; }
...
void ElementCoreDSC_Query::do_work(ElementPoint *point, TData *data) {
	if (sqlite) {
		...
		for (int i = 0; i < c; i++) dt << sqlite3_column_name(r, i);
		dt.dump();
...
void ElementCoreConvertor::read_var(ElementPoint *point, TData *data) { *data = result; }

//--------------------------------------------------------------------------------------------------

ElementCorePopup::ElementCorePopup(PackElement *pe, SDK *sdk, gdouble x, gdouble y) : Element(pe, sdk, x, y), ElementCore(pe, sdk, x, y) {
	// none
}

void ElementCorePopup::do_work(ElementPoint *point, TData *data) { popup->popup(3, 11113044); }
...
void ElementCorePopup::stopRun() { delete popup; }
...
void ElementTplWinControl::prepareForRun(Element **parent) {
	...
	if (op) {
		...
		if (props[4]->readInt() == 0) ctl->show();
		if (props[5]->readInt()) ctl->set_sensitive(false);
		if (!props[6]->isDefault()) ctl->set_tooltip_text(props[6]->readStr());
	}
}

void ElementTplWinControl::stopRun() { delete ctl; }
...
ElementTplParent::~ElementTplParent() { parent->on_add_element -= this; }

void ElementTplParent::callback(void *owner, CallbackType type, const void *data) { addElement((Element *)data); }
...
ElementTplForm::ElementTplForm(PackElement *pe, SDK *sdk, gdouble x, gdouble y) : Element(pe, sdk, x, y), ElementTplParent(pe, sdk, x, y) {
...
Widget *ElementTplForm::createWidget(Element **parent) {
	...
	if (!props[8]->isDefault()) form->set_modal(true);
	if (!props[9]->isDefault()) form->set_resizable(false);
...
void ElementTplForm::insertElement(ElementTplWinControl *e) { ((Container *)ctl)->add(*e->ctl); }
...
Widget *ElementTplDialog::createWidget(Element **parent) {
	...
	int i = 0;
	for (ArrayValue::iterator s = arr->begin(); s != arr->end(); s++) form->add_button(StockID(*(ustring *)s->data), i++);
...
void ElementTplDialog::insertElement(ElementTplWinControl *e) {
	...
	if (form)
		...
		vb = (Box *)ctl;
	vb->pack_start(*e->ctl, e->props.getByName("Expand")->isDefault(), e->props.getByName("Fill")->isDefault(),
...
ElementTplVBox::ElementTplVBox(PackElement *pe, SDK *sdk, gdouble x, gdouble y) : Element(pe, sdk, x, y), ElementTplParent(pe, sdk, x, y) {
	// do nothing
}
...
Widget *ElementTplVBox::createWidget(Element **parent) {
	*parent = this;
	...
	if (tpl->conf->cClass == "HCWE_VBox") return new VBox(false, props[7]->readInt());
	return new HBox(false, props[7]->readInt());
}

void ElementTplVBox::insertElement(ElementTplWinControl *e) {
	((VBox *)ctl)
			->pack_start(*e->ctl, e->props.getByName("Expand")->isDefault(), e->props.getByName("Fill")->isDefault(),
									 e->props.getByName("Padding")->readInt());
}
...
Widget *ElementTplExpander::createWidget(Element **parent) {
	*parent = this;
	...
	if (!props[9]->isDefault()) exp->set_expanded(true);
	return exp;
}

void ElementTplExpander::insertElement(ElementTplWinControl *e) { ((Expander *)ctl)->add(*e->ctl); }
...
void ElementTplButtonBox::insertElement(ElementTplWinControl *e) {
	...
	if (!e->props.getByName("Secondary")->isDefault()) ((ButtonBox *)ctl)->set_child_secondary(*e->ctl, true);
}
...
void ElementTplHandleBox::insertElement(ElementTplWinControl *e) { ((HandleBox *)ctl)->add(*e->ctl); }
...
Widget *ElementTplPaned::createWidget(Element **parent) {
	*parent = this;
	...
	if (tpl->conf->cClass == "HCWE_VPaned") return new VPaned();
	return new HPaned();
}

void ElementTplPaned::insertElement(ElementTplWinControl *e) {
	if (((Paned *)ctl)->get_child1())
		((Paned *)ctl)->pack2(*e->ctl, e->props.getByName("Resize")->isDefault(), e->props.getByName("Shrink")->isDefault());
	else
		((Paned *)ctl)->pack1(*e->ctl, e->props.getByName("Resize")->isDefault(), e->props.getByName("Shrink")->isDefault());
}
...
Widget *ElementTplButton::createWidget(Element **parent) {
	...
	if (!props[12]->isDefault()) {
		...
		if (!props[11]->isDefault()) btn->set_image_position((PositionType)(props[11]->readInt() + POS_LEFT));
...
void ElementTplEdit::on_edit_changed() { points[2]->on_event(((Entry *)ctl)->get_text()); }
...
void ElementTplEdit::read_var(ElementPoint *point, TData *data) { *data = ((Entry *)ctl)->get_text(); }

void ElementTplEdit::do_work(ElementPoint *point, TData *data) {
	if (point == points[0]) ((Entry *)ctl)->set_text(readString(data, points[5], NULL));
}
...
Widget *ElementTplMemo::createWidget(Element **parent) {
	...
	if (!props[8]->isDefault()) t->get_buffer()->set_text(props[8]->readStr());
	if (!props[9]->isDefault()) t->set_editable(false);
...
void ElementTplMemo::on_edit_changed() { points[4]->on_event(t->get_buffer()->get_text()); }
...
void ElementTplLabel::do_work(ElementPoint *point, TData *data) {
	if (point->name == "doCaption") ((Label *)ctl)->set_text(data ? data->toStr() : ustring());
}
...
void ElementTplScrollBar::on_change_value() { points[1]->on_event((int)((Scrollbar *)ctl)->get_value()); }

void ElementTplScrollBar::read_var(ElementPoint *point, TData *data) { *data = ((Scrollbar *)ctl)->get_value(); }

void ElementTplScrollBar::do_work(ElementPoint *point, TData *data) {
	if (point == points[0]) ((Scrollbar *)ctl)->set_value(readReal(data, points[3], NULL));
}
...
void ElementTplListBox::read_var(ElementPoint *point, TData *data) {
	if (point->name == "Text") {
		StringList lst;
		for (TreeNodeChildren::iterator n = list->children().begin(); n != list->children().end(); n++) lst << (*n)[listColumns.text];
		*data = lst.text();
	} else if (point->name == "Count")
...
void ElementTplListBox::setText(const ustring &text) {
	...
	lst = text;
	list->clear();
	for (StringList::iterator l = lst.begin(); l != lst.end(); l++) (*list->append())[listColumns.text] = *l;
}
...
Widget *ElementTplImage::createWidget(Element **parent) {
	...
	if (props[7]->readInt() != 2 || props[8]->readInt() != 2) {
		ScrolledWindow *sc = new ScrolledWindow();
		sc->set_policy((PolicyType)(props[7]->readInt() + POLICY_ALWAYS), (PolicyType)(props[8]->readInt() + POLICY_ALWAYS));
		sc->add(*image);
		image->show();
		return sc;
	}
...
void ElementTplFileChooserButton::on_file_set() { points[0]->on_event(((FileChooserButton *)ctl)->get_filename()); }
...
void ElementTplColorButton::on_color_set() { points[0]->on_event(((ColorButton *)ctl)->get_color().to_string()); }
...
Widget *ElementTplComboBox::createWidget(Element **parent) {
	...
	if (!props[8]->isDefault()) {
		...
		for (StringList::iterator l = list.begin(); l != list.end(); l++) cb->append(*l);
	}
